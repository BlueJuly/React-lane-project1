...
class App extends React.Component {
	...
	applyAction(id) {
		let newActions = this.state.actions.map(a=>{ 
			return Object.assign({}, a, { applied: id == a.id ? true : a.applied });
		});
		this.setState({
			actions: newActions
		});
	...
	unapplyAction(id) {
		let newActions = this.state.actions.map(a=>{ 
			return Object.assign({}, a, { applied: id == a.id ? false : a.applied });
		});
		this.setState({
			actions: newActions
		});
	...
	resetActions(){
		let newActions = this.state.actions.map(a=>{
			return Object.assign({}, a, { applied: false });
		});
	...
	computedStyle() {
		...
		let computedStyle = this.state.actions
			                  ...
			                  .reduce((previous, current) => { 
			                  	return Object.assign({}, previous, { [current.type]: current.action }) 
			                  }, imgStyle);
		...
		return (
			<Grid>
			    ...
			    <Row className="show-grid">
			      ...
			      <Col xs={6} md={3}>
			      	<Button onClick={this.resetActions.bind(this)}>Reset</Button>
			      </Col>
			    </Row>
			</Grid>
		);
